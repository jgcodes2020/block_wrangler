...
def _flag_values(mapping: list[MapEntry]):
            ...
            match key:
                case (name, value):
                    ...
                    if name not in flag_values:
                        flag_values[name] = {}
                    if value not in flag_values[name]:
                        flag_values[name][value] = []
                    flag_values[name][value].append(entry['id'])
                case str(name):
                    # boolean-valued flag
                    if name not in flag_values:
                        flag_values[name] = {}
                    if True not in flag_values[name]:
                        flag_values[name][True] = []
                    flag_values[name][True].append(entry['id'])
...
class BlockMapping:
    ...
    def render_encoder(self):
        ...
        for entry in self.mapping:
            lines.append(f"# {', '.join(str(x) for x in entry['flags'])}")
            lines.append(f"block.{entry['id']} = {entry['blocks'].render()}")
            lines.append("")
        return "\n".join(lines)
    
    def render_decoder(self):
        flag_values = _flag_values(self.mapping)
        ...
        for flag_name, flag in self.flags.items():
            lines.extend(flag.render_decoder(flag_values[flag_name], self.config))
            lines.append('')
            lines.append('')
